...
exports.submitPlacement = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  await db.ref(`rooms/${roomId}/private/${myUid}/board`).set(privateBoard);
  ...
  const [myBoardSnap, oppBoardSnap] = await Promise.all([
    db.ref(`rooms/${roomId}/private/${myUid}/board`).get(),
    db.ref(`rooms/${roomId}/private/${oppUid}/board`).get()
  ]);
...
exports.movePiece = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  if (!target) {
    ...
    await db.ref(`rooms/${roomId}/private/${uid}/board`).transaction((priv) => {
      if (!priv) return priv;
  ...
  const [attackerPieceSnap, defenderPieceSnap] = await Promise.all([
    db.ref(`rooms/${roomId}/private/${uid}/board/${fromKey}`).get(),
    db.ref(`rooms/${roomId}/private/${defenderUid}/board/${toKey}`).get()
  ]);
  ...
  if (attackerPiece === "star" || defenderPiece === "star") {
    ...
    await Promise.all([
      db.ref(`rooms/${roomId}/private/${uid}/board/${fromKey}`).remove(),
      db.ref(`rooms/${roomId}/private/${defenderUid}/board/${toKey}`).remove()
    ]);
...
exports.duelFold = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  const winnerPieceSnap = await db.ref(`rooms/${roomId}/private/${winnerUid}/board/${winnerPosKey}`).get();
  ...
  if (winnerSide === d.attacker) {
    await db.ref(`rooms/${roomId}/private/${attackerUid}/board/${fromKey}`).remove();
    await db.ref(`rooms/${roomId}/private/${attackerUid}/board/${toKey}`).set(winnerPiece);
    await db.ref(`rooms/${roomId}/private/${defenderUid}/board/${toKey}`).remove();
  } else {
    await db.ref(`rooms/${roomId}/private/${attackerUid}/board/${fromKey}`).remove();
  }
...
exports.duelCall = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  const [attackerSnap, defenderSnap] = await Promise.all([
    db.ref(`rooms/${roomId}/private/${attackerUid}/board/${fromKey}`).get(),
    db.ref(`rooms/${roomId}/private/${defenderUid}/board/${toKey}`).get()
  ]);
  ...
  if (outcome === "draw") {
    await Promise.all([
      db.ref(`rooms/${roomId}/private/${attackerUid}/board/${fromKey}`).remove(),
      db.ref(`rooms/${roomId}/private/${defenderUid}/board/${toKey}`).remove()
    ]);
  } else if (outcome === "a") {
    await db.ref(`rooms/${roomId}/private/${attackerUid}/board/${fromKey}`).remove();
    await db.ref(`rooms/${roomId}/private/${attackerUid}/board/${toKey}`).set(attackerPiece);
    await db.ref(`rooms/${roomId}/private/${defenderUid}/board/${toKey}`).remove();
  } else {
    await db.ref(`rooms/${roomId}/private/${attackerUid}/board/${fromKey}`).remove();
  }
...
exports.rematch = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  await Promise.all([
    hostUid ? db.ref(`rooms/${roomId}/private/${hostUid}/board`).remove() : Promise.resolve(),
    guestUid ? db.ref(`rooms/${roomId}/private/${guestUid}/board`).remove() : Promise.resolve()
  ]);
